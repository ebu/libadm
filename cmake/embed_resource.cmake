...
function(embed_resource)
    ...
    set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS ${EMBED_RESOURCE_FILES})

    set(byte_array_template "const char file_@FILE_COUNT@[] = { @FILE_BYTE_ARRAY@ }")
    set(lookup_template "if (fileName == \"@FILE_NAME@\") { stream.write(file_@FILE_COUNT@, @ARRAY_SIZE@)\; return true\; }")
    ...
    foreach(file_path ${EMBED_RESOURCE_FILES})
        ...
        # add 0x prefix and , separator
        # TRANSFORM only valid in cmake 3.14+ or we could just do
        # list(TRANSFORM file_bytes PREPEND "0x")
        # list(JOIN file_bytes "," FILE_BYTE_ARRAY)
        ...
        # add the trailing semicolon here as it gets
        # interpreted as a list seperator if in template
    ...
    # generate files
    configure_file("${current_dir}/embedded_resource.hpp.in" "${CMAKE_CURRENT_BINARY_DIR}/${EMBED_BASE_NAME}.hpp" @ONLY)
    configure_file("${current_dir}/embedded_resource.cpp.in" "${CMAKE_CURRENT_BINARY_DIR}/${EMBED_BASE_NAME}.cpp" @ONLY)
endfunction()
